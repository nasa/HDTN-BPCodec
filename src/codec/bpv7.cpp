...
namespace hdtn {
    static uint8_t _bpv7_eid_decode(bpv7_ipn_eid* eid, char* buffer, size_t offset, size_t bufsz) {
        ...
        static uint8_t _bpv7_eid_encode(bpv7_ipn_eid* eid, char* buffer, size_t offset, size_t bufsz) {
        uint64_t len = 0;
        buffer += offset;
        bufsz -= offset;
        uint8_t index = 1;  // set to 1 to skip the array indicator byte - it's always an array of length "2"
        ...
        if(type == BPV7_EID_SCHEME_DTN) {
            eid->type = BPV7_EID_SCHEME_DTN;
            index ++;
            index += cbor_encode_uint(&len, (uint8_t *)(&buffer[index]), 0, bufsz - index);
            if(len > (bufsz - index)) {
                printf("Bad string length: %llu\n", len);
                return 0;  // make sure we're not reading off the end of our assigned buffer here ...
            }
            ...
            // jump past array header byte
            index += 2;
            index += cbor_encode_uint(&eid->node, (uint8_t*)(buffer + index), 0, bufsz);
            index += cbor_encode_uint(&eid->service, (uint8_t*)(buffer + index), 0, bufsz);
            return index;
        }
        return 0;
    }
    ...
    uint32_t bpv7_canonical_block_encode(bpv7_canonical_block* block, char* buffer, size_t offset, size_t bufsz) {
        ...
        uint64_t tmp;
        uint8_t res = cbor_encode_uint(&tmp, (uint8_t *)buffer, offset + index, bufsz);
        index += res;
        if(0 == res) {
            return 0;
        }
        tmp = block->block_type;
        res = cbor_encode_uint(&tmp, (uint8_t *)buffer, offset + index, bufsz);
        index += res;
        if(0 == res) {
            return 0;
        }
    ...
    uint32_t bpv7_primary_block_encode(bpv7_primary_block* primary, char* buffer, size_t offset, size_t bufsz) {
        ...
        res = _bpv7_eid_encode(&primary->dst, buffer, index, bufsz);
        if(res == 0) {
            printf("Decoding EID (dst) failed.\n");
            return 0;
        }
        index += res;
        res = _bpv7_eid_encode(&primary->src, buffer, index, bufsz);
        if(res == 0) {
            printf("Decoding EID (src) failed.\n");
            return 0;
        }
        index += res;
        res = _bpv7_eid_encode(&primary->report, buffer, index, bufsz);
        if(res == 0) {
